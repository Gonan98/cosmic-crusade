...
namespace RTSEngine
{
    public class BuildingManager : MonoBehaviour
    {
        ...
        public void CreatePlacedInstance(Building buildingPrefab, Vector3 placementPosition, float yEulerAngle, Border buildingCenter, int factionID, List<Vector2Int> gridPositionList = null, bool placedByDefault = false, bool factionCapital = false)
        {
            if (GameManager.MultiplayerGame == false)
            { //if it's a single player game.
                CreatePlacedInstanceLocal(buildingPrefab, placementPosition, yEulerAngle, buildingCenter, factionID, gridPositionList, placedByDefault, factionCapital); //place the building
            }
            else
            { //in case it's a multiplayer game:
                ...
                var building = JsonUtility.ToJson(buildingPrefab, true);
                InputManager.SendInput(newInput, buildingPrefab, buildingCenter?.building); //send input to input manager
            }
        }

        //creates an instance of a building that is instantly placed:
        public Building CreatePlacedInstanceLocal(Building buildingPrefab, Vector3 placementPosition, float yEulerAngle, Border buildingCenter, int factionID, List<Vector2Int> gridPositionList = null, bool placedByDefault = false, bool factionCapital = false)
        {
            ...
            Building buildingInstance = PhotonNetwork.Instantiate(buildingPrefab.name, placementPosition, Quaternion.Euler(buildingEulerAngles)).GetComponent<Building>(); //create instance

            buildingInstance.PlacedByDefault = placedByDefault;

            buildingInstance.Init(gameMgr, factionID, false, buildingCenter, factionCapital, gridPositionList); //init the building
