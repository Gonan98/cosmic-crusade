...
namespace RTSEngine
{
    public class Building : FactionEntity, IPunInstantiateMagicCallback
	{
        ...
        [SerializeField]
        private int buildingWidth = 2; //grid width to positioning building on grid
        public int GetWidth() { return buildingWidth; }

        [SerializeField]
        private int buildingLength = 2; //grid width to positioning building on grid
        public int GetLength() { return buildingLength; }
        ...
        public List<Vector2Int> gridPositionList { get; set; }
        ...
        {
            if (regulatorData == null) regulatorData = new NPCBuildingRegulatorDataSingle();
		...
		public void OnPhotonInstantiate(PhotonMessageInfo info)
		{
			...
			if (!info.photonView.IsMine)
			{
				/* (unit != null)
				{
					Debug.Log("SSSSSSSSSSSSS" + unit.GetName());
				}*/
				//var entity = FindObjectOfType<Entity>();
				//unit = entity;
				//var gameManager = FindObjectOfType<GameManager>();
				//var unit = info.photonView.gameObject;
				Init(FindObjectOfType<GameManager>(), 1, true, FindObjectOfType<Border>(), false, null);

                if (!FindObjectOfType<GameManager>().InPeaceTime())
                    HealthComp.AddHealth(1, this);
                // Añadir sobre el peaceTime para que no sucede a cada rato
                //if(Resources.FindObjectsOfTypeAll(typeof(Building))?.Length < 1) {
                //    this.HealthComp.CurrHealth = this.HealthComp.MaxHealth;
				//}
		...
		public void Init(GameManager gameMgr, int fID, bool free, Border buildingCenter, bool factionCapital,  List<Vector2Int> gridPositionList = null)
        {
        ...
        public void InitPlacementInstance(GameManager gameMgr, int fID, List<Vector2Int> gridPositionList = null, Border buildingCenter = null)
        {
            PlacementInstance = true; //init as placement instance

            Init(gameMgr, fID, false, null, false, gridPositionList); //init the building
        ...
        public void OnBuilt()
        {
            ...
            UpdateTaskLauncherTasks(this.TaskLauncherComp);
        ...
        public void UpdateTaskLauncherTasks(TaskLauncher taskLauncher)
        {
            ...
            for (int taskID = 0; taskID < taskLauncher.GetTasksCount(); taskID++) //go through all tasks
            {
                ...
                task.showBeforeBuilt = true;
                // gameMgr.TaskMgr.Status.ToggleTask(task.GetCode(), factionID, true);
            }
        }
