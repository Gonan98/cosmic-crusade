...
namespace RTSEngine
{
    ...
    public class GameManager : MonoBehaviourPunCallbacks, IOnEventCallback
    {
...
#endif
        public const byte WinOrLoseEventCode = 6;
        [SerializeField]
        private string mainMenuScene = "Lobby"; //Main menu scene name, this is the scene that will be loaded when the player decides to leave the game.
        ...
        void Update()
        {
            ...
            int countUnits = FindObjectsOfType<Unit>().Where(e => e.GetComponent<PhotonView>().IsMine).Count();
            int countBuildings = FindObjectsOfType<Building>().Where(e => e.GetComponent<PhotonView>().IsMine).Count();

            if (GameState == GameState.running && countUnits == 0 && countBuildings == 0)
            {
                LooseGame();
            }
		}
        ...
        public void LooseGame()
        {
            ...
            PhotonNetwork.RaiseEvent(WinOrLoseEventCode, null, raiseEventOptions, SendOptions.SendReliable);
        }
        ...
        public void OnEvent(EventData photonEvent)
        {
            ...
            if (eventCode == WinOrLoseEventCode)
            {
                WinGame();
            }
        }
