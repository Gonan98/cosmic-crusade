    ...
    public class FactionSlot
    {
        ...
        public void SpawnFactionEntities (GameManager gameMgr)
        {
            ...
            if (capitalBuilding != null)
            {
				...
				{
					CapitalPosition = new Vector3(17.0f, 12.0f, 137.0f);
                    this.name = PhotonNetwork.LocalPlayer.NickName;
                }
                else { 
				    //Debug.Log("1 IF");
                ...
                {
                    //Debug.Log("2IF");
                    GameObject.Destroy(this.capitalBuilding.gameObject); //destroy the default capital and spawn another one:
                    capitalBuilding = gameMgr.BuildingMgr.CreatePlacedInstanceLocal(
                        typeInfo.GetCapitalBuilding(),
                        CapitalPosition,
                        PhotonNetwork.IsMasterClient ? 180.0f : typeInfo.GetCapitalBuilding().transform.rotation.eulerAngles.y,
                        null, ID, null, true, true);
                }
            ...
            foreach(DefaultFactionEntity defaultFactionEntity in defaultFactionEntities) //go through the default faction entities
            {
                    ...
                    || defaultFactionEntity.factionTypes.Any(t => t == typeInfo)) //or the faction slot's type is specified in the array
                {
					...
					int index = 0;
                    Vector3[] positions = {
                        new Vector3(8f,12f,136f),
                        new Vector3(9f,12f,138f),
                        new Vector3(9f,12f,140f),
                        new Vector3(10f,12f,142f),
                        new Vector3(11f,12f,144f),
                    };
					foreach (FactionEntity instance in defaultFactionEntity.instances)
                    {
                        ...
                        else 
                        {
							...
							newUnit = gameMgr.UnitMgr.CreateUnit(instance as Unit, instance.gameObject.transform.position, instance.gameObject.transform.rotation, instance.gameObject.transform.position, 0, null, false, false);
							Object.DestroyImmediate(instance.gameObject); //destroy the default capital and spawn another one:
						}

						//if this is a unit, then update the population and update the faction limits:
						if(instance != null) FactionMgr.UpdateLimitsList(instance.GetCode(), instance.GetCategory(), true);
						if (instance != null && instance.Type == EntityTypes.unit)
                        {
							//Debug.Log("TEST: "+instance.name);
							//Debug.Log("8IF");
							Unit defaultUnit = (Unit)instance;
                            UpdateCurrentPopulation(defaultUnit.GetPopulationSlots());
                        }

						if (newUnit != null) FactionMgr.UpdateLimitsList(newUnit.GetCode(), newUnit.GetCategory(), true);
						if (newUnit != null && newUnit.Type == EntityTypes.unit)
						{
							//Debug.Log("TEST: " + newUnit.name);
							//Debug.Log("8IF");
							Unit defaultUnit = (Unit)newUnit;
							UpdateCurrentPopulation(defaultUnit.GetPopulationSlots());
						}
                        index++;
					}
