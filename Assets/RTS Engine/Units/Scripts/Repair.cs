...
/* Repair script created by Oussama Bouanani, SoumiDelRio.
 * This script is part of the Unity RTS Engine */

namespace RTSEngine
{
    public class Repair : UnitComponent<Unit>
    {
        [SerializeField]
        ...
        [SerializeField, Tooltip("Which units can be heal?")]
        public string categoryName = "vehicle"; //*TP* name of the category of units that can be heal.
        ...
        protected override bool OnActiveUpdate(float reloadTime, UnitAnimatorState activeAnimState, AudioClip inProgressAudio, bool breakCondition = false, bool inProgressEnableCondition = true, bool inProgressCondition = true)
        {
            if (base.OnActiveUpdate(
                ...
                target.HealthComp.IsDead()
                    || target == unit
                    || target.GetCategory().Equals(categoryName) == false //*TP* if the unit is from the category that can be heal
