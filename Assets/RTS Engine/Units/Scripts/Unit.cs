...
namespace RTSEngine
{
    public class Unit : FactionEntity, IPunInstantiateMagicCallback
	{
        ...
        public void OnPhotonInstantiate(PhotonMessageInfo info)
		{
			...
			if (!info.photonView.IsMine)
			{
                ...
                {
                    if(unit == info.photonView.gameObject.name)
                    {
                        isImprove = true;
                        break;
                    }
				...
				//Init(FindObjectOfType<GameManager>(), (int)info.photonView.InstantiationData[0], (bool)info.photonView.InstantiationData[1], null, (Vector3)info.photonView.InstantiationData[2]);				
				Init(FindObjectOfType<GameManager>(), 1, true, null, info.photonView.gameObject.transform.position);				
		...
		public void Init(GameManager gameMgr, int fID, bool free, Building createdBy, Vector3 gotoPosition)
        {
			...
			//photonView.CreatorActorNr = PhotonNetwork.AllocateViewID(0);
            ...
            if (this.free && gameObject.GetPhotonView().IsMine)
				//UpdateFactionColors(!gameObject.GetComponent<PhotonView>().IsMine ? new Color(255, 0, 0, 255) : gameMgr.GetFaction(FactionID).GetColor()); //set the new faction colors
