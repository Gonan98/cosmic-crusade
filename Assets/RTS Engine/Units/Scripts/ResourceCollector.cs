...
namespace RTSEngine
{
	public class ResourceCollector : UnitComponent<Resource> {
        ...
        public override bool Stop()
        {
            ...
            photonView.RPC("RPC_StopCollectorParticles", RpcTarget.Others);
        ...
        {
            //if (!this.photonView.IsMine) return false;
        ...
        [PunRPC]
        protected virtual void RPC_ShowCollectParticles(string resourceName)
        {
            var particles = GetComponentsInChildren<ParticleSystem>();
            int index = resourceName == "Gas" ? 5 : 0;
        ...
        protected override void OnInProgressEnabled(float reloadTime, UnitAnimatorState activeAnimState, AudioClip inProgressAudio)
        {
            ...
            //Debug.Log("RECURSO:" + target.GetResourceType().name);

            var particles = GetComponentsInChildren<ParticleSystem>();
            var recurso = target.GetResourceType().name;
            int index = recurso == "Gas" ? 5 : 0;
            particles[index].Play();

            var photonView = GetComponent<PhotonView>();
            photonView.RPC("RPC_ShowCollectParticles", RpcTarget.Others, target.GetResourceType().name);

            //Debug.Log("Particulas: " + particles.Length);
		...
		public void UpdateDropOffBuilding ()
		{
            ...
            foreach(Building b in unit.FactionMgr.GetDropOffBuildings()) //go through all faction's drop off buildings
            {
                ...
                if (b.IsBuilt == true && b.DropOffComp.CanDrop(target.GetResourceType().Key)) //if this drop off building is completely built and it accepts the target resource
                    ...
                    {
                        //Debug.Log($"[UpdateDropOffBuilding] -> Target is null : {target == null}");
                        //Debug.Log($"[UpdateDropOffBuilding] -> B is null : {b == null}");
                        //Debug.Log($"[UpdateDropOffBuilding] -> closestBuilding is null : {closestBuilding == null}");
                        closestBuilding = b;
        ...
        [PunRPC]
        protected virtual void RPC_StopCollectorParticles()
        {
            foreach (var particle in GetComponentsInChildren<ParticleSystem>())
            {
                particle.Stop();
