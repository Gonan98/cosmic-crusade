...
namespace RTSEngine.EntityComponent
{
    ...
    public abstract class AttackEntity : MonoBehaviourPun, IEntityComponent
    {
        ...
        {
            get { return isActive && FactionEntity != null && !FactionEntity.EntityHealthComp.IsDead(); }
            set
            {
                isActive = value;
        ...
        {
            //Debug.Log($"AttackEntity ErrorMessage: My action number es {FactionEntity.photonView.OwnerActorNr}");
            //Debug.Log($"AttackEntity ErrorMessage: Enemy action number es {potentialTarget.photonView.OwnerActorNr}");
            //Debug.Log($"AttackEntity ErrorMessage : {IsTargetValid(potentialTarget)}");
        ...
        protected virtual void OnNoTargetUpdate()
        {
            ...
            if (!RTSHelper.IsLocalPlayer(FactionEntity) //this is the local player..
                ...
                return;
            */
            if (gameMgr.InPeaceTime() //game is still in peace time
                || !engageInRange //can't search for target
                || (engageInRangeIdleOnly && !IsIdle())) //or unit is just currently not idle
                return;
            
            //Debug.Log($"PhotonView CreatorActor : {base.photonView.CreatorActorNr}");
            //Debug.Log($"PhotonView OwnerActor : {base.photonView.OwnerActorNr}");
            //Debug.Log($"PhotonView ActorNumber : {PhotonNetwork.LocalPlayer.ActorNumber}");
            /*if (!base.photonView.IsMine //this is the local player..
                || gameMgr.InPeaceTime() //game is still in peace time
                || !engageInRange //can't search for target
                || (engageInRangeIdleOnly && !IsIdle())) //or unit is just currently not idle
            {
                //Debug.Log("[AttackEntitty/OnNoTargetUpdate] No es mi unidad");
                return;
            }*/
        ...
        {
            /*var pv = FactionEntity.GetComponent<PhotonView>();
            Debug.Log("Faction Entity: " + FactionEntity.name);
        ...
        /*[PunRPC]
        protected virtual void RPC_OnAttackPerformed(PhotonView photonView, Vector3 position)
        {
            var target = FindObjectsOfType<FactionEntity>().Where(e => e.GetComponent<PhotonView>().ViewID == photonView.ViewID).First();
            CustomEvents.OnAttackPerformed(this, target, position);
            gameMgr.AudioMgr.PlaySFX(FactionEntity.AudioSourceComp, attackAudio.Fetch(), false);
        }*/

        [PunRPC]
        protected virtual void RPC_OnIndirectAttackUpdate(int viewId, Vector3 position, bool direct)
        {
            //Target = FindObjectsOfType<FactionEntity>().Where(e => e.photonView.ViewID == viewId).First();
            Target = PhotonView.Find(viewId).GetComponent<FactionEntity>();
            if (direct)
            {
            ...
            if (attackObjectLauncher.OnIndirectAttackUpdate())
                gameMgr.AudioMgr.PlaySFX(FactionEntity.AudioSourceComp, attackAudio.Fetch(), false);
        }
        ...
        protected virtual void OnTargetUpdate()
        {
            ...
            if (triggered)
            {
                ...
                {
                    //Debug.Log("Entro en DIRECT=TRUE");
                    base.photonView.RPC("RPC_OnIndirectAttackUpdate", RpcTarget.Others, Target.photonView.ViewID, GetTargetPosition(), direct);
                    attackPerformedEvent.Invoke();
                    //not an area attack:
                    ...
                    OnAttackComplete();
                    return;
                    //FactionEntity.GetComponent<PhotonView>().RPC("RPC_OnAttackPerformed", RpcTarget.Others, Target, GetTargetPosition());
                }

                bool onIndirectAttackUpdate = attackObjectLauncher.OnIndirectAttackUpdate();
                /*else*/
                if (onIndirectAttackUpdate)
                {
                    //Debug.Log("Entro en DIRECT=FALSE");
                    base.photonView.RPC("RPC_OnIndirectAttackUpdate", RpcTarget.Others, Target.photonView.ViewID, GetTargetPosition(), direct);
                    gameMgr.AudioMgr.PlaySFX(FactionEntity.AudioSourceComp, attackAudio.Fetch(), false);

                } //indirect attack ? -> must launch attack objects
            }
